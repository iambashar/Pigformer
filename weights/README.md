...
|---|---|
| `pigformer_fold0.pt` | Fold-0 PigFormer (Stage 2). Single-fold checkpoint usable via `evaluate.py`. |

Per-fold ensemble checkpoints can be saved under `results/four_fold_*/foldK/best.pt`
and combined with `evaluate_ensemble.py` for the 3.87 mm headline number reported in the paper.
